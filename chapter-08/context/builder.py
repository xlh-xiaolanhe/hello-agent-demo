...
import tiktoken
from hello_agents import HelloAgentsLLM, Message
...
class ContextPacket:
    """上下文信息包"""
    content: str
    # field(default_factory=... 确保可变默认值在每次实例化时都是独立的新对象，避免多个实例共享同一对象导致的 bug。
...
class ContextConfig:
    ...
    reserve_ratio: float = 0.15  # 生成余量 （10% - 20%）确保系统指令等关键信息始终有足够的空间，不会被其他信息挤占。
