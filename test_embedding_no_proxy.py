"""测试DashScope Embedding API - 禁用代理版本"""
import os
import requests
...
print("🚀 测试API调用...")
try:
    resp = requests.post(url, headers=headers, json=payload, timeout=30, proxies={"http": None, "https": None})
    ...
    if resp.status_code >= 400:
        ...
        data = resp.json()
        print(f"  ✅ API调用成功")
        print(f"  Response结构: {list(data.keys())}")
